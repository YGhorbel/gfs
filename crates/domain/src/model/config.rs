...
impl GfsConfig {
    pub fn load(repo_path: &Path) -> Result<Self, RepoError> {
        ...
        let config = toml::from_str(&content).map_err(|e| RepoError::InvalidConfig(e.to_string()))?;
        Ok(config)
    }

    pub fn save(&self, repo_path: &Path) -> Result<(), RepoError> {
        let config_path = repo_path.join(GFS_DIR).join(CONFIG_FILE);
        let content = toml::to_string_pretty(self).map_err(|e| RepoError::InvalidConfig(e.to_string()))?;
